...
if NET == 0:
    print("VGG16")
    net = VGG('VGG16', num_classes = 7).to(device)
else:
    print("AlexNet")
    net = AlexNet(num_classes = 7).to(device)
...
def test(model,test_loader, criterion,optim,filename, epochs):
    ...
    for i,(images,_, labels,_) in enumerate(tqdm(test_loader)):
        images, labels = images.to(device), labels.to(device)
        outputs = model(images)
        loss = criterion(outputs, labels)
...
plt.suptitle("VGG16 - 7Class Classification")
plt.savefig('/home/aminul/unr/imgs/vgg_7class.png',dpi=300)
