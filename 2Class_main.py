...
def train(model,train_loader,criterion,optim,epochs):
    ...
    for i,(images,_,_, labels) in enumerate(tqdm(train_loader)):
...
def test(model,test_loader, criterion,optim, filename, epochs):
    ...
    for i,(images,_,_, labels) in enumerate(tqdm(test_loader)):
        images, labels = images.to(device), labels.to(device)
        outputs = model(images)
        loss = criterion(outputs, labels)
...
plt.suptitle("VGG16 - 2 Class Classification")
plt.savefig('/home/aminul/unr/imgs/vgg_2class.png',dpi=300)
